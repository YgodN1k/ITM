  ...
  return (
    <article className="rounded-[2rem] bg-[#121212] p-3 shadow-frame">
      <div className="aspect-[399/250] overflow-hidden rounded-[1.65rem] bg-black">
        <img
          src={withBasePath(build.image)}
          alt={`Сборка ${build.sku}`}
          className="h-full w-full object-contain"
        />
      </div>
      <div className="px-1 pb-2 pt-6">
        <p className="text-[1.75rem] leading-none text-white/90">ID:{build.sku}</p>
...
export default function HomeClient({ builds }: { builds: Build[] }) {
  const buildSlides = Array.from({ length: 5 }, (_, id) => ({ id, items: builds }));
  ...
  const selectedBuild = selectedBuildId
    ? builds.find((item) => item.id === selectedBuildId) ?? null
    : null;
  ...
  const handleOrderBuild = (id: string) => {
  ...
  const handleContactSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (isSpammyName(name)) {
      setFormMessage("Пожалуйста, укажите имя без ссылок и спам-символов.");
      return;
    }
            ...
            <div className="flex flex-col py-2 lg:py-9">
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <button
                  type="button"
                  onClick={() => {
                    const jivo = (window as Window & { jivo_api?: { open: () => void } }).jivo_api;
                    if (jivo) {
                      jivo.open();
                    } else {
                  ...
                  <div key={slide.id} className="min-w-full">
                    <div className="grid gap-5 lg:grid-cols-2 xl:grid-cols-4">
                      {slide.items.map((build) => (
                        <BuildCard
                          key={`${slide.id}-${build.id}`}
                          build={build}
                          onOrder={handleOrderBuild}
                        />
                      ))}
                    </div>
                  </div>
          ...
          <div className="mx-auto max-w-4xl">
            ...
            <form className="mt-8 space-y-4" onSubmit={handleContactSubmit}>
              ...
              <input
                ...
                className="w-full rounded-full border border-transparent bg-[#222] px-6 py-4 text-white outline-none transition placeholder:text-white/40 focus:border-accent"
              />
              <input type="hidden" name="build_id" value={selectedBuildId ?? ""} />
              {selectedBuild ? (
                <p className="text-center text-sm text-white/70">
                  Заказываете сборку <span className="font-medium text-accent">{selectedBuild.sku}</span>
                  {" "}
                  <button
                    type="button"
